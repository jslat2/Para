...
public class Database extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE todo(ID INTEGER, userNum TEXT, clientNum INTEGER, todoNum Integer, task TEXT, done TEXT, yearDue INTEGER, monthDue INTEGER, dayDue INTEGER);");
    }
    ...
    public int getToDoDay(Database db, int currentUser, int clientListPosition, int currentToDoPosition){
        SQLiteDatabase sq = db.getReadableDatabase();
        String[] col = {"dayDue"};
        Cursor cr = sq.query("todo WHERE userNum = " + currentUser + " AND clientNum = " + getClientID(db, currentUser, clientListPosition), col, null, null, null, null, "yearDue asc, monthDue asc, dayDue asc");
        cr.moveToPosition(currentToDoPosition);
        return cr.getInt(0);
    }

    public int getToDoMonth(Database db, int currentUser, int clientListPosition, int currentToDoPosition){
        SQLiteDatabase sq = db.getReadableDatabase();
        String[] col = {"monthDue"};
        Cursor cr = sq.query("todo WHERE userNum = " + currentUser + " AND clientNum = " + getClientID(db, currentUser, clientListPosition), col, null, null, null, null, "yearDue asc, monthDue asc, dayDue asc");
        cr.moveToPosition(currentToDoPosition);
    ...
    public int getToDoYear(Database db, int currentUser, int clientListPosition, int currentToDoPosition){
        SQLiteDatabase sq = db.getReadableDatabase();
        String[] col = {"yearDue"};
        Cursor cr = sq.query("todo WHERE userNum = " + currentUser + " AND clientNum = " + getClientID(db, currentUser, clientListPosition), col, null, null, null, null, "yearDue asc, monthDue asc, dayDue asc");
        cr.moveToPosition(currentToDoPosition);
    ...
    public String getToDoTask(Database db, int currentUser, int clientListPosition, int currentToDoPosition){
        SQLiteDatabase sq = db.getReadableDatabase();
        String[] col = {"task"};
        Cursor cr = sq.query("todo WHERE userNum = " + currentUser + " AND clientNum = " + getClientID(db, currentUser, clientListPosition), col, null, null, null, null, "yearDue asc, monthDue asc, dayDue asc");
        cr.moveToPosition(currentToDoPosition);
    ...
    public String getToDoComplete(Database db, int currentUser, int clientListPosition, int currentToDoPosition){
        SQLiteDatabase sq = db.getReadableDatabase();
        String[] col = {"done"};
        Cursor cr = sq.query("todo WHERE userNum = " + currentUser + " AND clientNum = " + getClientID(db, currentUser, clientListPosition), col, null, null, null, null, "yearDue asc, monthDue asc, dayDue asc");
        cr.moveToPosition(currentToDoPosition);
    ...
    public void saveToDo(Database db, int currentUser, int currentClientPosition, String completed, int year, int month, int day, String task){
        SQLiteDatabase sq = db.getWritableDatabase();
        ContentValues cv = new ContentValues();

        cv.put("ID", getNextToDoNum(db));
        cv.put("userNum", currentUser);
        cv.put("clientNum", getClientID(db, currentUser, currentClientPosition));
        cv.put("task", task);
        cv.put("done", completed);
        cv.put("yearDue", year);
        cv.put("monthDue", month);
        cv.put("dayDue", day);

        long k = sq.insert("todo", null, cv);
    }

    public void editToDo(Database db, int currentUser, int currentClientPosition, int currentToDoPosition, String completed, int year, int month, int day, String task){
        SQLiteDatabase sq = db.getWritableDatabase();
        ContentValues cv = new ContentValues();
        ...
        long k = sq.update("todo", cv, "ID = " + getToDoID(db, currentUser, currentClientPosition, currentToDoPosition), null);
    }
    ...
    public int getToDoID(Database db, int user, int clientListPosition, int toDoPosition) {
        Cursor cr = db.getClientTodos(db, user, clientListPosition);
        cr.moveToPosition(toDoPosition);
        return cr.getInt(1);
    }
    ...
    public int getNextToDoNum(Database db){
        int x = 0;
        String[] col = new String[1];
        col[0] = "ID";
        SQLiteDatabase sq = db.getReadableDatabase();
        Cursor cr = sq.query("todo", col, null, null, null, null, null);
        if(cr.moveToFirst()) {
            while(!cr.isAfterLast()){
                if(cr.getInt(0) > x){
                    x = cr.getInt(0);
